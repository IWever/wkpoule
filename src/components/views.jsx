...
import {
  GROUP_MATCHES, ALL_TEAMS, KO_STRUCTURE, NL_STAGES, SURPRISE_TEAMS,
  PTS_SURPRISE, TOP_TEAMS, PTS_TOP_OUT, PLAYERS_BY_COUNTRY,
  PTS_GROUP, PTS_STANDING, PTS_KO, PTS_EXTRA, FLAG, GROUPS, TEAM_GROUP,
} from "../data/tournamentData";
import {
  calcPoints, calcGroupMatchPts, buildRichKOSlots, deriveSurpriseStage,
  deriveTopOuts, deriveGroupStandingsFromResults, groupsAllFilled,
  persist, resolveSlotRich, fmtDateTime,
} from "../pouleEngine";
...
function MyOverview({ user, state, onEditGroup, onEditExtra, onEditKO }) {
  ...
  const ranked = [...state.users].sort(
    (a, b) => calcPoints(b, state.results, state.koResults) - calcPoints(a, state.results, state.koResults)
  );
  ...
  const KO_DATES = { r32:"2026-07-01", r16:"2026-07-05", qf:"2026-07-10", sf:"2026-07-14", "3rd":"2026-07-18", final:"2026-07-19" };
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:1, minWidth:90 }}>
            {pw ? (
              <div style={{ display:"flex", gap:3, alignItems:"center" }}>
                ...
                <span style={{ fontWeight:700, color:"var(--orange)", background:"rgba(240,136,62,.1)", borderRadius:4, padding:"1px 7px", fontSize:13 }}>{FLAG[pw] || ""} {pw}</span>
              </div>
  ...
  function MatchRow({ m }) {
    ...
    return (
      <div onClick={canCompareMatch ? () => setCompareMatch(m) : undefined}
        style={{ marginBottom:6, ...S.card(), padding:"8px 10px", border:`1px solid ${borderColor}`,
          background: isGroupF ? "rgba(240,136,62,.03)" : "var(--card)", cursor: canCompareMatch ? "pointer" : "default" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:5 }}>
          <span style={{ fontSize:10, color:"var(--muted)" }}>{m.dt ? fmtDateTime(m.dt) : ""}</span>
          <div style={{ display:"flex", gap:6, alignItems:"center" }}>
            <span style={{ fontSize:10, color: isGroupF ? "var(--orange)" : "var(--accent)", fontWeight:700,
              background: isGroupF ? "rgba(240,136,62,.1)" : "rgba(88,166,255,.1)", borderRadius:4, padding:"1px 6px" }}>
              Groep {m.group}
            </span>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:4, fontSize:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:1, minWidth:90 }}>
            <div style={{ display:"flex", gap:3, alignItems:"center" }}>
              <span style={{ fontSize:10, color:"var(--muted)" }}>Jij:</span>
              <span style={{ fontWeight:700, color: isGroupF ? "var(--orange)" : "var(--accent)",
                background: isGroupF ? "rgba(240,136,62,.1)" : "rgba(88,166,255,.1)", borderRadius:4, padding:"1px 7px", fontSize:13 }}>
          ...
          {res && <span style={{ minWidth:52, textAlign:"right", fontWeight:700, fontSize:11, color: res.pts > 0 ? "var(--green)" : "var(--red)" }}>
    ...
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {!state.extraFrozen
            ? <button style={S.btn("var(--green)")} onClick={onEditExtra}>🔮 Extra vragen</button>
            : <button style={{ ...S.btn("var(--card2)"), border:"1px solid var(--border)" }} onClick={onEditExtra}>👁️ Extra vragen</button>}
          {!state.groupFrozen
            ? <button style={S.btn()} onClick={onEditGroup}>⚽ Groepsfase</button>
            : <button style={{ ...S.btn("var(--card2)"), border:"1px solid var(--border)" }} onClick={onEditGroup}>👁️ Groepsfase</button>}
          {koAvailable && (!state.koFrozen
            ? <button style={S.btn("var(--orange)")} onClick={onEditKO}>⚔️ KO-fase</button>
            : <button style={{ ...S.btn("var(--card2)"), border:"1px solid var(--border)" }} onClick={onEditKO}>👁️ KO-fase</button>)}
        </div>
      </div>
      ...
      <div style={{ marginBottom:22 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {(() => {
            ...
            const surpriseGroup = pred.surpriseTeam ? (TEAM_GROUP[pred.surpriseTeam] || null) : null;
            const topOutGroup = pred.topOut ? (TEAM_GROUP[pred.topOut] || null) : null;
            ...
            return [
              ...
              { label:"🌟 Verrassing", value:pred.surpriseTeam, groupLetter:surpriseGroup, type:"surprise", stage:surpriseStage, stagePts:surprisePts },
              { label:"💥 Topland uit", value:pred.topOut, groupLetter:topOutGroup, type:"topout", correct:topOutCorrect, known:topOutKnown },
            ].map((item) => (
              <div key={item.label} style={{ ...S.card(), flex:1, minWidth:120 }}>
                <div style={{ fontSize:11, color:"var(--muted)", marginBottom:4, display:"flex", alignItems:"center", justifyContent:"space-between", gap:4 }}>
                  ...
                  {item.groupLetter && (
                    <span style={{ fontSize:10, fontWeight:700,
                      color: item.groupLetter === "F" ? "var(--orange)" : "var(--muted)",
                      background: item.groupLetter === "F" ? "rgba(240,136,62,.12)" : "rgba(255,255,255,.06)",
                ...
                <div style={{ fontWeight:700, fontSize:13 }}>
                  {item.value ? (FLAG[item.value] ? `${FLAG[item.value]} ${item.value}` : item.value) : <span style={{ color:"var(--muted)" }}>–</span>}
                </div>
                {item.type === "simple" && item.known && (
                  <div style={{ fontSize:12, marginTop:4, color: item.value === item.actual ? "var(--green)" : "var(--red)", fontWeight:700 }}>
                    {item.value === item.actual ? `+${item.pts} ✓` : "✗"}
                  </div>
                )}
                {item.type === "topscorer" && item.known && (() => {
                  const winners = Array.isArray(item.actual) ? item.actual : [item.actual];
                  const correct = winners.includes(item.value);
                  return <div style={{ fontSize:12, marginTop:4, color: correct ? "var(--green)" : "var(--red)", fontWeight:700 }}>{correct ? `+${item.pts} ✓` : "✗"}</div>;
                })()}
                {item.type === "yellowcards" && item.known && (
                  <div style={{ fontSize:12, marginTop:4, color: item.correct ? "var(--green)" : "var(--red)", fontWeight:700 }}>{item.correct ? `+${item.pts} ✓` : "✗"}</div>
                )}
                ...
                {item.type === "topout" && item.value && item.known && (
                  <div style={{ fontSize:12, marginTop:4, color: item.correct ? "var(--green)" : "var(--red)", fontWeight:700 }}>{item.correct ? `+${PTS_TOP_OUT} ✓` : "✗"}</div>
                )}
              </div>
            ));
          })()}
      ...
      <div style={{ marginBottom:22 }}>
        {last5.length > 0 && (
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:12, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:8 }}>Laatste wedstrijden</div>
            {last5.map((m) => m.isKO ? <KOMatchRow key={m.id} m={m} /> : <MatchRow key={m.id} m={m} />)}
          </div>
        )}
        {next5.length > 0 && (
          <div>
            <div style={{ fontSize:12, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:8 }}>Volgende wedstrijden</div>
            {next5.map((m) => m.isKO ? <KOMatchRow key={m.id} m={m} /> : <MatchRow key={m.id} m={m} />)}
          </div>
        )}
    ...
    <div>
      ...
      <div style={{ ...S.card(), marginBottom:14 }}>
        ...
        <div style={{ fontSize:13, color:"var(--muted)", lineHeight:1.8 }}>
          ...
          <strong style={{ color:"var(--text)" }}>Groepsfase</strong> en — zodra de admin dit openzet — de{" "}
      ...
      <div style={{ ...S.card(), marginBottom:14 }}>
        {[
          ...
          ["⚽ Topscorer", "Kies een land en vervolgens de speler die de meeste doelpunten scoort. De volledige selectie is beschikbaar, gesorteerd op doelpunten in de kwalificatie.", PTS_EXTRA.topScorer],
          ["🇳🇱 Hoe ver komt Nederland?", "Voorspel in welke ronde Nederland uitvalt — of kampioen wordt.", PTS_EXTRA.nlStage],
          ["🟨 Meeste gele kaarten", "Welk land heeft aan het einde van het toernooi de meeste gele kaarten ontvangen?", PTS_EXTRA.yellowCards],
        ...
        <div style={{ borderBottom:"1px solid var(--border)", paddingBottom:10 }}>
          ...
          <div style={{ fontSize:12, color:"var(--muted)", marginBottom:8 }}>Kies een van de 12 laagst geklasseerde landen:</div>
          <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
            {Object.entries(PTS_SURPRISE).filter(([,v]) => v > 0).map(([s,p]) => (
      ...
      <div style={{ ...S.card(), marginBottom:14 }}>
        {[
          ...
          ["Juist doelpuntenverschil", "Bv. jij zegt 3–1, het wordt 2–0", PTS_GROUP.diff],
          ["Juiste winnaar of gelijkspel", "Je hebt de juiste richting, maar verschil klopt niet", PTS_GROUP.winner],
        ...
        {[["Team in jouw top-2 én gaat echt door", PTS_STANDING.qualified], ["Zelfde positie (#1 of #2 exact correct)", PTS_STANDING.qualifiedCorrectPos]].map(([desc, pts]) => (
      ...
      <div style={{ ...S.card(), marginBottom:14 }}>
        {[["Zestiende finale",PTS_KO.r32],["Achtste finale",PTS_KO.r16],["Kwartfinale",PTS_KO.qf],["Halve finale",PTS_KO.sf],["Finale",PTS_KO.final]].map(([ronde,schema]) => (
        ...
        <div style={{ fontSize:11, color:"var(--muted)", marginTop:8 }}>💡 De exacte stand na 90 min is een bonus bovenop de winnaar-punten.</div>
      </div>
      ...
      {ranked.map((u, i) => {
        ...
        return (
          <div key={u.id} onClick={clickable ? () => onCompare(u) : undefined}
            style={{ display:"flex", alignItems:"center", gap:14, ...S.card(), marginBottom:8,
...
function StandWithCompare({ state, currentUser }) {
  ...
  const defaultComp = (() => {
    ...
    const first = competitions.find((c) => (currentUser.competitionIds || []).includes(c.id));
  ...
  const filteredUsers = (!selectedComp || competitions.length === 0)
    ? state.users
    : state.users.filter((u) => (u.competitionIds || []).includes(selectedComp));
  return (
    <div>
      {competitions.length > 0 && (
        <div style={{ marginBottom:18 }}>
          <div style={{ fontSize:11, color:"var(--muted)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8, fontWeight:700 }}>Competitie</div>
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>

            {competitions.map((c) => (
              <button key={c.id} onClick={() => setSelectedComp(c.id)} style={{ padding:"6px 14px", borderRadius:20, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"var(--font)",
                border:`1px solid ${selectedComp === c.id ? "var(--accent)" : "var(--border)"}`,
                background: selectedComp === c.id ? "var(--accent)" : "var(--bg)",
                color: selectedComp === c.id ? "#fff" : "var(--text)" }}>
                {c.name}<span style={{ marginLeft:6, fontSize:11, opacity:0.7 }}>({state.users.filter((u) => (u.competitionIds||[]).includes(c.id)).length})</span>
              </button>
            ))}
          </div>
        </div>
      )}
      <Standings state={{ ...state, users: filteredUsers }} currentUserId={currentUser?.id} onCompare={setComparePlayer} />
      {comparePlayer && currentUser && <PlayerCompare me={currentUser} other={comparePlayer} state={state} onClose={() => setComparePlayer(null)} />}
    </div>
  );
}
...
function PwReveal({ u }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:4 }}>
      ...
      <button onClick={() => setShow((s) => !s)} style={{ background:"none", border:"1px solid var(--border)", borderRadius:5, color:"var(--muted)", cursor:"pointer", fontSize:10, padding:"2px 6px", fontFamily:"var(--font)" }}>
        {show ? "verberg" : "toon"}
      </button>
    </div>
  );
...
function CompetitionsAdmin({ state, setState }) {
  ...
  function addComp() {
    ...
    setState((s) => { const ns = { ...s, competitions: [...(s.competitions||[]), { id, name: newName.trim(), hidden: false, hiddenRegistration: false }] }; persist(ns); return ns; });
    setNewName("");
  }

  function removeComp(id) {
    if (!confirm("Competitie verwijderen?")) return;
    setState((s) => { const ns = { ...s, competitions:(s.competitions||[]).filter((c)=>c.id!==id), users:s.users.map((u)=>({...u,competitionIds:(u.competitionIds||[]).filter((cid)=>cid!==id)})) }; persist(ns); return ns; });
  }

  function toggleUserInComp(userId, compId) {
    setState((s) => {
      const ns = { ...s, users: s.users.map((u) => {
        if (u.id !== userId) return u;
        const ids = u.competitionIds || [];
        return { ...u, competitionIds: ids.includes(compId) ? ids.filter((id)=>id!==compId) : [...ids, compId] };
      })};
  ...
  return (
    <div>
      <div style={{ ...S.card(), marginBottom:16 }}>
        <div style={{ fontWeight:600, fontSize:14, marginBottom:10 }}>Nieuwe competitie aanmaken</div>
        <div style={{ display:"flex", gap:8 }}>
          <input style={{ ...S.input, flex:1 }} placeholder="Naam (bijv. Familie poule)" value={newName} onChange={(e)=>setNewName(e.target.value)} onKeyDown={(e)=>{ if(e.key==="Enter") addComp(); }} />
          <button style={{ ...S.btn("var(--green)"), whiteSpace:"nowrap" }} onClick={addComp}>+ Aanmaken</button>
        </div>
      </div>
      {competitions.length === 0 && <p style={{ color:"var(--muted)", fontSize:13 }}>Nog geen competities.</p>}
      {competitions.map((comp) => {
        const members = state.users.filter((u) => (u.competitionIds||[]).includes(comp.id));
        const nonMembers = state.users.filter((u) => !(u.competitionIds||[]).includes(comp.id));
        return (
          <div key={comp.id} style={{ ...S.card(), marginBottom:14 }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12, flexWrap:"wrap", gap:8 }}>
              <div>
                <div style={{ fontWeight:700, fontSize:15 }}>{comp.name}</div>
                <div style={{ fontSize:12, color:"var(--muted)", marginTop:2 }}>{members.length} deelnemer{members.length!==1?"s":""}</div>
              </div>
              <div style={{ display:"flex", gap:6 }}>
                <button onClick={() => setState((s) => { const ns = { ...s, competitions: s.competitions.map((c) => c.id === comp.id ? { ...c, hidden: !c.hidden } : c) }; persist(ns); return ns; })}
                  title="Zichtbaarheid in de stand"
                  style={{ background: comp.hidden ? "rgba(240,136,62,.12)" : "rgba(88,166,255,.08)", border:`1px solid ${comp.hidden ? "var(--orange)" : "var(--border)"}`, borderRadius:6, color: comp.hidden ? "var(--orange)" : "var(--muted)", padding:"4px 10px", cursor:"pointer", fontSize:12, fontFamily:"var(--font)" }}>
                  {comp.hidden ? "🏅 Verborgen" : "🏅 Zichtbaar"}
                </button>
                <button onClick={() => setState((s) => { const ns = { ...s, competitions: s.competitions.map((c) => c.id === comp.id ? { ...c, hiddenRegistration: !c.hiddenRegistration } : c) }; persist(ns); return ns; })}
                  title="Zichtbaarheid bij registratie"
                  style={{ background: comp.hiddenRegistration ? "rgba(240,136,62,.12)" : "rgba(88,166,255,.08)", border:`1px solid ${comp.hiddenRegistration ? "var(--orange)" : "var(--border)"}`, borderRadius:6, color: comp.hiddenRegistration ? "var(--orange)" : "var(--muted)", padding:"4px 10px", cursor:"pointer", fontSize:12, fontFamily:"var(--font)" }}>
                  {comp.hiddenRegistration ? "📝 Verborgen" : "📝 Zichtbaar"}
                </button>
                <button onClick={() => removeComp(comp.id)} style={{ background:"none", border:"1px solid var(--border)", borderRadius:6, color:"var(--muted)", padding:"4px 10px", cursor:"pointer", fontSize:12, fontFamily:"var(--font)" }}>🗑 Verwijderen</button>
              </div>
            </div>
            {members.length > 0 && (
              <div style={{ marginBottom:10 }}>
                <div style={{ fontSize:11, color:"var(--muted)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:6 }}>Deelnemers</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {members.map((u) => (
                    <div key={u.id} style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(88,166,255,.1)", border:"1px solid rgba(88,166,255,.3)", borderRadius:20, padding:"4px 10px 4px 12px", fontSize:13 }}>
                      <span style={{ color:"var(--accent)", fontWeight:600 }}>{u.name}</span>
                      <button onClick={()=>toggleUserInComp(u.id,comp.id)} style={{ background:"none", border:"none", cursor:"pointer", color:"var(--muted)", fontSize:14, padding:"0 2px", lineHeight:1 }}>×</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {nonMembers.length > 0 && (
              <div>
                <div style={{ fontSize:11, color:"var(--muted)", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.07em", marginBottom:6 }}>Toevoegen</div>
                <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                  {nonMembers.map((u) => (
                    <button key={u.id} onClick={()=>toggleUserInComp(u.id,comp.id)} style={{ background:"var(--bg)", border:"1px solid var(--border)", borderRadius:20, padding:"4px 12px", fontSize:13, color:"var(--text)", cursor:"pointer", fontFamily:"var(--font)" }}>+ {u.name}</button>
                  ))}
                </div>
              </div>
            )}
...
function AdminHome({ state }) {
  ...
  const koMatches = KO_STRUCTURE.map((m) => ({ ...m, dt: KO_DATES[m.round]+"T20:00", isKO:true }));
  const allPlayed = [...groupMatches, ...koMatches]
    .filter((m) => m.isKO ? state.koResults[m.id]?.played : state.results[m.id]?.played)
    .sort((a,b) => (a.dt||"").localeCompare(b.dt||""));
  ...
  return (
    <div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:10, marginBottom:22 }}>
        <div style={{ ...S.card(), textAlign:"center" }}>
          <div style={{ fontFamily:"var(--font-display)", fontSize:26, color:"var(--accent)" }}>{state.users.length}</div>
          <div style={{ fontSize:11, color:"var(--muted)", textTransform:"uppercase", letterSpacing:"0.08em" }}>Deelnemers</div>
        </div>
        <div style={{ ...S.card(), textAlign:"center" }}>
          <div style={{ fontFamily:"var(--font-display)", fontSize:26, color:"var(--accent)" }}>{allPlayed.length}</div>
          <div style={{ fontSize:11, color:"var(--muted)", textTransform:"uppercase", letterSpacing:"0.08em" }}>Gespeeld</div>
        </div>
        <div style={{ ...S.card(), textAlign:"center" }}>
          <div style={{ fontFamily:"var(--font-display)", fontSize:26, color:"var(--accent)" }}>{totalMatches - allPlayed.length}</div>
          <div style={{ fontSize:11, color:"var(--muted)", textTransform:"uppercase", letterSpacing:"0.08em" }}>Resterend</div>
        </div>
      </div>
      <div style={{ ...S.card(), marginBottom:18, background:"rgba(88,166,255,.04)" }}>
        <div style={{ fontSize:12, fontWeight:700, color:"var(--muted)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:10 }}>Status</div>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {[{label:"Groepsfase",key:"groupFrozen",icon:"⚽"},{label:"Extra vragen",key:"extraFrozen",icon:"🔮"},{label:"KO open",key:"koOpen",icon:"⚔️",invert:true},{label:"KO bevroren",key:"koFrozen",icon:"🔒"}].map(({label,key,icon,invert}) => {
            const active = state[key];
            const isGood = invert ? active : !active;
            return (
              <div key={key} style={{ display:"flex", alignItems:"center", gap:6, background: isGood ? "rgba(63,185,80,.1)" : "rgba(248,81,73,.08)", border:`1px solid ${isGood ? "rgba(63,185,80,.3)" : "rgba(248,81,73,.2)"}`, borderRadius:8, padding:"6px 12px", fontSize:12 }}>
                <span>{icon}</span>
                <span style={{ fontWeight:600, color: isGood ? "var(--green)" : "var(--red)" }}>{label}</span>
                <span style={{ color: isGood ? "var(--green)" : "var(--red)", fontSize:11 }}>{invert ? (active ? "✓ open" : "✗ dicht") : (active ? "✗ bevroren" : "✓ open")}</span>
              </div>
            );
          })}
        </div>
      </div>
      <div style={{ fontSize:12, fontWeight:700, color:"var(--accent)", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:10 }}>Laatste gespeelde wedstrijden</div>
      {last5.length === 0 ? (
        <div style={{ ...S.card(), color:"var(--muted)", fontSize:13, textAlign:"center" }}>Nog geen wedstrijden gespeeld.</div>
      ) : last5.map((m) => {
        const r = m.isKO ? state.koResults[m.id] : state.results[m.id];
        const isGroupF = !m.isKO && m.group === "F";
        return (
          <div key={m.id} style={{ ...S.card(), marginBottom:8, padding:"10px 14px",
            border:`1px solid ${isGroupF ? "rgba(240,136,62,.3)" : m.isKO ? "rgba(240,136,62,.25)" : "var(--border)"}`,
            background: isGroupF ? "rgba(240,136,62,.03)" : "var(--card)" }}>
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
              <span style={{ fontSize:10, color:"var(--muted)" }}>{m.dt ? fmtDateTime(m.dt) : ""}</span>
              <span style={{ fontSize:10, fontWeight:700, color: m.isKO||isGroupF ? "var(--orange)" : "var(--accent)", background: m.isKO||isGroupF ? "rgba(240,136,62,.1)" : "rgba(88,166,255,.1)", borderRadius:4, padding:"1px 6px" }}>
                {m.isKO ? m.label : `Groep ${m.group} · R${m.round}`}
              </span>
            </div>
            <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:13 }}>
              <span style={{ flex:1, textAlign:"right", fontWeight:600 }}>{m.isKO ? (m.homeSlot||"?") : `${FLAG[m.home]||""} ${m.home}`}</span>
              <div style={{ textAlign:"center", minWidth:70 }}>
                {m.isKO ? (r?.winner ? (
                  <div>
                    <div style={{ fontWeight:900, fontSize:15, color:"var(--green)" }}>{r.home90!==undefined ? `${r.home90}–${r.away90}` : "–"}</div>
                    <div style={{ fontSize:11, color:"var(--muted)", marginTop:2 }}>✓ {FLAG[r.winner]||""} {r.winner}</div>
                  </div>
                ) : "–") : (
                  <span style={{ fontWeight:900, fontSize:18, color:"var(--text)", background:"rgba(255,255,255,.07)", borderRadius:6, padding:"2px 10px" }}>{r?.home}–{r?.away}</span>
                )}
              </div>
              <span style={{ flex:1, fontWeight:600 }}>{m.isKO ? (m.awaySlot||"?") : `${FLAG[m.away]||""} ${m.away}`}</span>
            </div>
          </div>
        );
      })}
    ...
    <div>
      ...
      <TabBar
        tabs={[
          ...
          { id:"competitions", label:`🏆 Competities (${(state.competitions||[]).length})` },
      ...
      {tab === "fase" && (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {[
            ...
            {key:"extraFrozen",icon:"🔮",label:"Extra vragen",desc:"Kampioen, topscorer, Nederland, gele kaarten, verrassing en topland"},
            {key:"koOpen",icon:"⚔️",label:"KO-fase openen",desc:"Deelnemers kunnen KO-voorspellingen invullen",invert:true},
            ...
            return (
              <div key={key} style={{ ...S.card(), display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:12 }}>
                ...
                <button onClick={() => upd({ [key]: !active })} style={{ ...S.btn(invert ? (active?"var(--green)":"var(--card2)") : (active?"var(--red)":"var(--green)")), padding:"8px 18px", fontSize:13, border:`1px solid ${invert?(active?"var(--green)":"var(--border)"):(active?"var(--red)":"var(--green)")}` }}>
                  {invert ? (active ? "✓ Open" : "Openzetten") : (active ? "🔒 Bevroren" : "🔓 Open")}
                </button>
              </div>
            );
          })}
      ...
      {tab === "competitions" && <CompetitionsAdmin state={state} setState={setState} />}
      ...
      {tab === "users" && (
        <div>
          ...
          {state.users.map((u) => {
            ...
            const extraFilled = [!!p.champion, !!p.topScorer, !!p.nlStage, !!p.yellowCards, !!p.surpriseTeam, !!p.topOut].filter(Boolean).length;
            const koFilled = KO_STRUCTURE.filter((m) => p.koWinners?.[m.id]).length;
            const userComps = (u.competitionIds||[]).map((cid) => (state.competitions||[]).find((c)=>c.id===cid)?.name).filter(Boolean);
            function Pill({label, filled, total, done, started, available}) {
              if (!available) return <span style={{ fontSize:11, color:"var(--muted)", background:"rgba(255,255,255,.04)", borderRadius:4, padding:"2px 7px" }}>{label} —</span>;
              ...
              return <span style={{ fontSize:11, color, background:bg, borderRadius:4, padding:"2px 7px", fontWeight:done?700:400 }}>{done?"✓":started?"…":"○"} {label} {filled}/{total}</span>;
            }
            return (
              <div key={u.id} style={{ ...S.card(), marginBottom:10 }}>
                ...
                {userComps.length > 0 && (
                  <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:6 }}>
                    {userComps.map((name) => <span key={name} style={{ fontSize:11, background:"rgba(88,166,255,.1)", border:"1px solid rgba(88,166,255,.2)", borderRadius:10, padding:"1px 8px", color:"var(--accent)" }}>{name}</span>)}
                  </div>
                )}
                <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                  <Pill label="Extra" filled={extraFilled} total={6} done={extraFilled===6} started={extraFilled>0} available={true} />
                  <Pill label="Groepsfase" filled={groupFilled} total={GROUP_MATCHES.length} done={groupFilled===GROUP_MATCHES.length} started={groupFilled>0} available={true} />
                  <Pill label="KO-fase" filled={koFilled} total={KO_STRUCTURE.length} done={koFilled===KO_STRUCTURE.length} started={koFilled>0} available={state.koOpen||state.fase==="ko"} />
                </div>
              </div>
            );
          })}
        ...
        <div>
          <div style={{ display:"flex", gap:5, flexWrap:"wrap", marginBottom:14 }}>
            {"ABCDEFGHIJKL".split("").map((g) => (
              <button key={g} onClick={() => setActiveGroup(g)} style={{ padding:"5px 13px", borderRadius:20,
                ...
                color: activeGroup===g ? "#fff" : g==="F" ? "var(--orange)" : "var(--text)", cursor:"pointer", fontSize:13, fontWeight:600 }}>
                Groep {g}
              </button>
            ))}
          </div>
          {(() => {
            ...
            if (!s || !s.table.some((r) => r.gp > 0)) return (
              <div style={{ ...S.card(), marginBottom:12, fontSize:12, color:"var(--muted)" }}>Nog geen wedstrijden gespeeld in Groep {activeGroup}.</div>
            );
            const allGroupPlayed = GROUP_MATCHES.filter((m)=>m.group===activeGroup).every((m)=>state.results[m.id]?.played);
            if (allGroupPlayed && s.winner && state.results[`GW_${activeGroup}`] !== s.winner) {
              setTimeout(() => setState((prev) => { const ns = { ...prev, results: { ...prev.results, [`GW_${activeGroup}`]: s.winner, [`GR_${activeGroup}`]: s.runnerUp } }; persist(ns); return ns; }), 0);
            }
            return (
              <div style={{ ...S.card(), marginBottom:12, border: isGroupF ? "1px solid rgba(240,136,62,.3)" : "1px solid var(--border)" }}>
                <div style={{ fontSize:11, fontWeight:700, color: isGroupF ? "var(--orange)" : "var(--green)", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:8 }}>
                  📊 Huidige stand Groep {activeGroup}{allGroupPlayed && <span style={{ marginLeft:8, color:"var(--accent)" }}>✓ Volledig gespeeld</span>}
                </div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 30px 34px 34px 34px 34px", gap:"0 4px", color:"var(--muted)", fontWeight:700, fontSize:11, textTransform:"uppercase", padding:"2px 4px", borderBottom:"1px solid var(--border)" }}>
                  <span>Team</span><span style={{textAlign:"center"}}>W</span><span style={{textAlign:"center"}}>Pts</span>
                  <span style={{textAlign:"center",color:"var(--green)"}}>GV+</span><span style={{textAlign:"center",color:"var(--red)"}}>GT-</span><span style={{textAlign:"center"}}>DS</span>
                </div>
                {s.table.map((r,i) => (
                  <div key={r.team} style={{ display:"grid", gridTemplateColumns:"1fr 30px 34px 34px 34px 34px", gap:"0 4px", alignItems:"center", padding:"5px 4px", background:i<2?"rgba(88,166,255,0.06)":"transparent", borderRadius:4, borderBottom:"1px solid rgba(48,54,61,.4)", fontSize:12 }}>
                    <span style={{ fontWeight:i<2?700:400, display:"flex", alignItems:"center", gap:5 }}>
                      ...
                      {FLAG[r.team]} {r.team}{i<2&&<span style={{ fontSize:10, color:i===0?"var(--green)":"var(--accent)", marginLeft:4 }}>→ KO</span>}
                    </span>
          ...
          {GROUP_MATCHES.filter((m)=>m.group===activeGroup).map((m) => {
            ...
            return (
              <div key={m.id} style={{ marginBottom:6 }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, ...S.card(), padding:"8px 12px",
                  border:`1px solid ${r.played ? (isGroupF?"rgba(240,136,62,.4)":"rgba(88,166,255,.4)") : "var(--border)"}`,
                  background: isGroupF ? "rgba(240,136,62,.03)" : "var(--card)" }}>
                  ...
                  <label style={{ fontSize:12, display:"flex", alignItems:"center", gap:4, cursor:"pointer", color:r.played?(isGroupF?"var(--orange)":"var(--accent)"):"var(--muted)", whiteSpace:"nowrap" }}>
      ...
      {tab === "ko" && (
        <div>
          <Alert msg="Vul de uitslag na 90 minuten in plus de officiële winnaar (na evt. verlenging/strafschoppen)." type="info" />
          {KO_STRUCTURE.map((m) => {
            const r = state.koResults[m.id] || {};
            const adminCtx = {
              ...
              userKoWinners: Object.fromEntries(KO_STRUCTURE.map((km) => [km.id, state.koResults[km.id]?.winner]).filter(([,v])=>v)),
            ...
            const homeCands = homeDesc?.type==="team"?[homeDesc.team]:homeDesc?.type==="two"?homeDesc.teams:[];
            const awayCands = awayDesc?.type==="team"?[awayDesc.team]:awayDesc?.type==="two"?awayDesc.teams:[];
              ...
              <div key={m.id} style={{ ...S.card(), marginBottom:10, border:`1px solid ${r.played?"rgba(88,166,255,.4)":"var(--border)"}` }}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap", borderTop:"1px solid var(--border)", paddingTop:8 }}>
                  ...
                  {useButtons ? (
                    ...
                    <select value={r.winner||""} onChange={(e)=>updKO(m.id,"winner",e.target.value)} style={{ flex:1, background:"var(--bg)", color:"var(--text)", border:"1px solid var(--border)", borderRadius:6, padding:"6px 10px", fontSize:13, fontFamily:"var(--font)" }}>
      ...
      {tab === "extra" && (
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div style={S.card()}>
            <div style={{ fontSize:13, color:"var(--muted)", marginBottom:6 }}>⚽ Officiële topscorer(s) — selecteer meerdere bij gelijkstand</div>
            <select value={state.results["TOP_SCORER_COUNTRY"]||""} onChange={(e) => setState((s) => { const ns = { ...s, results: { ...s.results, TOP_SCORER_COUNTRY: e.target.value, TOP_SCORER: [] } }; persist(ns); return ns; })}
            ...
            {state.results["TOP_SCORER_COUNTRY"] && (() => {
              const allPlayers = [...(PLAYERS_BY_COUNTRY[state.results["TOP_SCORER_COUNTRY"]]||[])].sort((a,b)=>b.kwal!==a.kwal?b.kwal-a.kwal:a.name.localeCompare(b.name));
              const currentWinners = Array.isArray(state.results["TOP_SCORER"]) ? state.results["TOP_SCORER"] : state.results["TOP_SCORER"] ? [state.results["TOP_SCORER"]] : [];
              function toggleScorer(name) {
                setState((s) => {
                  const cur = Array.isArray(s.results["TOP_SCORER"]) ? s.results["TOP_SCORER"] : s.results["TOP_SCORER"] ? [s.results["TOP_SCORER"]] : [];
                  const next = cur.includes(name) ? cur.filter((n)=>n!==name) : [...cur, name];
                  const ns = { ...s, results: { ...s.results, TOP_SCORER: next } }; persist(ns); return ns;
                });
              }
              return (
                <div>
                  <div style={{ fontSize:11, color:"var(--muted)", marginBottom:8 }}>Geselecteerd: {currentWinners.length===0?"geen":currentWinners.join(", ")}</div>
                  <div style={{ display:"flex", flexDirection:"column", gap:5, maxHeight:260, overflowY:"auto" }}>
                    {allPlayers.map((p) => {
                      const selected = currentWinners.includes(p.name);
                      return (
                        <button key={p.name} onClick={() => toggleScorer(p.name)}
                          style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"7px 12px", borderRadius:8, border:`2px solid ${selected?"var(--green)":"var(--border)"}`, background:selected?"rgba(63,185,80,.12)":"var(--bg)", color:"var(--text)", cursor:"pointer", fontSize:13, fontWeight:selected?700:400, fontFamily:"var(--font)", textAlign:"left" }}>
                          <span>{selected?"✓ ":""}{p.name}</span>
                          {p.kwal > 0 && <span style={{ fontSize:11, color:"var(--muted)" }}>{p.kwal} kwal. goals</span>}
                        </button>
                      );
                    })}
                  </div>
                  <div style={{ marginTop:10, borderTop:"1px solid var(--border)", paddingTop:10 }}>
                    <div style={{ fontSize:11, color:"var(--muted)", marginBottom:6 }}>Speler uit ander land toevoegen:</div>
                    <select value="" onChange={(e) => { if(!e.target.value) return; setState((s) => { const cur = Array.isArray(s.results["TOP_SCORER"])?s.results["TOP_SCORER"]:s.results["TOP_SCORER"]?[s.results["TOP_SCORER"]]:[]; if(cur.includes(e.target.value)) return s; const ns = {...s,results:{...s.results,TOP_SCORER:[...cur,e.target.value]}}; persist(ns); return ns; }); }}
                      style={{ background:"var(--bg)", color:"var(--text)", border:"1px solid var(--border)", borderRadius:6, padding:"7px 10px", fontSize:13, width:"100%", fontFamily:"var(--font)" }}>
                      <option value="">— voeg speler uit ander land toe —</option>
                      {Object.keys(PLAYERS_BY_COUNTRY).sort().map((country) => (
                        <optgroup key={country} label={`${FLAG[country]||""} ${country}`}>
                          {PLAYERS_BY_COUNTRY[country].slice().sort((a,b)=>b.kwal-a.kwal).map((pl) => (
                            <option key={pl.name} value={pl.name}>{pl.name}{pl.kwal>0?` (${pl.kwal} kwal. goals)`:""}</option>
                          ))}
                        </optgroup>
                      ))}
                    </select>
          ...
          <div style={S.card()}>
            <div style={{ fontSize:13, color:"var(--muted)", marginBottom:10 }}>🟨 Land met meeste gele kaarten (na toernooi)</div>
            <select value={state.results["YELLOW_CARDS"]||""} onChange={(e) => setState((s) => { const ns = { ...s, results: { ...s.results, YELLOW_CARDS: e.target.value } }; persist(ns); return ns; })}
              style={{ background:"var(--bg)", color:"var(--text)", border:"1px solid var(--border)", borderRadius:6, padding:"8px 12px", fontSize:14, width:"100%", fontFamily:"var(--font)" }}>
              <option value="">— nog niet bekend —</option>
              {ALL_TEAMS.map((t) => <option key={t} value={t}>{FLAG[t]||"🏳️"} {t}</option>)}
            </select>
          </div>
          {(() => {
            const topOuts = deriveTopOuts(state.results);
            const surpriseProgress = SURPRISE_TEAMS.map((team) => ({ team, stage: deriveSurpriseStage(team, state.koResults) })).filter((s) => s.stage);
            return (
              <>
                <div style={{ ...S.card(), background:"rgba(88,166,255,.04)" }}>
                  ...
                  {topOuts.length === 0 ? <span style={{ fontSize:12, color:"var(--muted)", fontStyle:"italic" }}>Nog geen volledig gespeelde groepen met toplands</span> : (
                    <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                      {topOuts.map((t) => <span key={t} style={{ background:"rgba(248,81,73,.15)", border:"1px solid var(--red)", borderRadius:6, padding:"3px 10px", fontSize:13, fontWeight:700, color:"var(--red)" }}>{FLAG[t]||"🏳️"} {t}</span>)}
                    </div>
                  )}
                </div>
                <div style={{ ...S.card(), background:"rgba(88,166,255,.04)" }}>
                  <div style={{ fontSize:13, color:"var(--muted)", marginBottom:8 }}>🌟 Verrassing-landen in KO-fase (automatisch)</div>
                  {surpriseProgress.length === 0 ? <span style={{ fontSize:12, color:"var(--muted)", fontStyle:"italic" }}>Nog geen KO-wedstrijden gespeeld</span> : (
