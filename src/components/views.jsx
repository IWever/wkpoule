...
function StandWithCompare({ state, currentUser }) {
  ...
  const filteredUsers = selectedComp === "all" ? state.users : state.users.filter((u) => (u.competitionIds || []).includes(selectedComp));
  return (
    <div>
      {competitions.length > 0 && (
        <div style={{ marginBottom:18 }}>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            <button onClick={() => setSelectedComp("all")} style={{ padding:"6px 14px", borderRadius:20, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"var(--font)",
              border:`1px solid ${selectedComp === "all" ? "var(--accent)" : "var(--border)"}`,
              background: selectedComp === "all" ? "var(--accent)" : "var(--bg)",
              color: selectedComp === "all" ? "#fff" : "var(--text)" }}>🌍 Iedereen</button>
            {competitions.map((c) => (
...
function CompetitionsAdmin({ state, setState }) {
  ...
  function addComp() {
    ...
    setState((s) => { const ns = { ...s, competitions: [...(s.competitions||[]), { id, name: newName.trim() }] }; persist(ns); return ns; });
    setNewName("");
  }
