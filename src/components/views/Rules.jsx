...
import { PTS_GROUP, PTS_STANDING, PTS_KO, PTS_EXTRA, PTS_SURPRISE } from "../data/tournamentData";
...
function SectionTitle({ children }) {
  return (
    <div style={{ fontSize: 13, fontWeight: 700, color: "var(--accent)", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 18 }}>
      {children}
    </div>
  );
}
...
function RuleRow({ title, desc, pts, noBorder }) {
  return (
    <div style={{ borderBottom: noBorder ? "none" : "1px solid var(--border)", paddingBottom: 10, marginBottom: 10 }}>
    ...
    <>
      ...
      <div style={{ ...S.card(), marginBottom: 14 }}>
        {[
          ...
          ["⚽ Topscorer", "Kies een land en vervolgens de speler die de meeste doelpunten scoort.", PTS_EXTRA.topScorer],
      ...
      <div style={{ ...S.card(), marginBottom: 14 }}>
        {[
          ...
          <div key={cat} style={{ display: "flex", gap: 10, borderBottom: "1px solid var(--border)", paddingBottom: 8, marginBottom: 8 }}>
            ...
            <div style={{ fontWeight: 900, fontSize: 18, color: pts > 0 ? "var(--accent)" : "var(--muted)", minWidth: 40, textAlign: "right" }}>
              {pts > 0 ? `+${pts}` : "-"}
            </div>
          </div>
        ))}
